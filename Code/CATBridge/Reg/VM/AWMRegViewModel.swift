...
public struct AWMRegViewModel: WLBaseViewModel {
    ...
    init(_ input: WLInput ,disposed: DisposeBag) {
        ...
        let logined: Driver<WLBaseResult> = input.loginTaps.withLatestFrom(usernameAndVcode).flatMapLatest {
            ...
            switch awmCheckUsernameAndVCode($0.0, vcode: $0.1) {
            case .ok:
                
                return awmDictResp(AWMApi.swiftLogin($0.0, code: $0.1))
                    .mapObject(type: AWMAccountBean.self)
                    .map({ AWMAccountCache.default.saveAccount(acc: $0) }) // 存储account
                    .map({ $0.toJSON()})
